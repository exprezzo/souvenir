...
if ( !empty($this->total) ){
	...
	for($i=0; $i<$this->numPaginas; $i++){
			...
			echo '<li class="  "><a class="'.$clase.'" href="'.$APP_PATH.$_idioma.'/portal/publicaciones/mostrar/'.$categoriaActual['id'].'?pagina='.($i+1).'#titulo_categoria">'.($i+1).'</a></li>';
...
<ul class="productos">
	<?php
		...
		foreach($this->publicaciones as $prod){
			// $link=
			$titulo=$prod['titulo'.$sufijo];
			$descripcion=$prod['contenido'.$sufijo];
			$imagen=(empty($prod['imagen']) )?  $WEB_BASE.'imagenes/no-image.jpg' : $WEB_BASE.'imagenes/'.$prod['imagen'];
			// $precio=$prod['precio'.$sufijo];
			if ($_idioma=='en'){
				$target=$APP_PATH.$_idioma.'/portal/publicaciones/mostrar/'.$categoriaActual['id'].'/'.str_replace(' ','_',$titulo);
			}else{
				$target=$APP_PATH.$_idioma.'/portal/publicaciones/mostrar/'.$categoriaActual['id'].'/'.str_replace(' ','_',$titulo);
			}			
		...
		?>	
			<li style="border: #eeeeee 2px solid; margin-right:22px; margin-bottom:19px;">
				<a href="<?php echo $APP_PATH.$_idioma.'/portal/publicaciones/detalle/'.$prod['id'].'/'.str_replace(' ','_',$prod['titulo'.$sufijo]); ?>">
